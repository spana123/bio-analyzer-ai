...
with col1:
    ...
    # สร้าง Tab เพื่อให้เลือกวิธีส่งรูปภาพ
    tab_upload, tab_camera = st.tabs(["📁 อัปโหลดรูปภาพ", "📷 เปิดกล้องถ่ายรูป"])
    
    with tab_upload:
        file_input = st.file_uploader("เลือกรูปภาพน้ำหมัก", type=["jpg", "png", "jpeg"], key="file")
        
    with tab_camera:
        camera_input = st.camera_input("ถ่ายรูปน้ำหมักชีวภาพ", key="camera")

    # กำหนดตัวแปรหลักที่จะนำไปใช้ (ถ้ามีรูปจากช่องทางใดช่องทางหนึ่ง)
    image_source = file_input if file_input is not None else camera_input
    ...
    if image_source:
        ...
        st.image(image_thumbnail, caption="ภาพที่ใช้ประมวลผล", width=300)
...
if st.button("🚀 เริ่มการวิเคราะห์คุณภาพ"):
    if image_source is not None and interpreter is not None:  # เปลี่ยนจาก uploaded_file เป็น image_source
        with st.spinner('กำลังประมวลผล...'):
            ...
            detected_fruit = class_names[idx].strip()
            confidence = float(prediction[idx]) # ค่าความเชื่อมั่น

            # --- ส่วนตรวจสอบความมั่นใจ (Threshold) ---
            threshold = 0.70  # เกณฑ์ความมั่นใจ 70%

            if confidence < threshold:
                # แสดงหน้าจอแจ้งเตือนสวยๆ เมื่อ AI ไม่มั่นใจ
                st.warning("### 🔍 ไม่สามารถระบุชนิดน้ำหมักได้ชัดเจน")
                st.markdown(f"""
                    <div style="background-color: #FFF3E0; padding: 25px; border-radius: 15px; border: 2px dashed #FF9800; text-align: center;">
                        <h2 style="color: #E65100; margin-top: 0;">⚠️ พบข้อผิดพลาดในการวิเคราะห์</h2>
                        <p style="color: #5D4037; font-size: 18px;">
                            ระบบมีความเชื่อมั่นเพียง <b>{confidence*100:.2f}%</b> ซึ่งต่ำกว่าเกณฑ์มาตรฐาน
                        </p>
                        <hr style="border: 0.5px solid #FFCC80;">
                        <p style="font-size: 16px; color: #795548;">
                            <b>คำแนะนำ:</b> โปรดถ่ายภาพในที่สว่าง หรือตรวจสอบว่าวัตถุคือ 1 ใน 5 ชนิดน้ำหมักที่ระบบรองรับ
                        </p>
                    </div>
                """, unsafe_allow_html=True)
            
            else:
                # --- 5.2 การวิเคราะห์สถานะเมื่อ AI มั่นใจเกินเกณฑ์ ---
                if detected_fruit in fruit_standards:
                    ...
                    st.write(f"ความเชื่อมั่นของระบบ AI: **{confidence*100:.2f}%**")
                    ...
                    if ph_pass and ec_pass and odor_pass:
                        ...
                        if not ph_pass: st.info(f"📌 pH {ph_input:.2f} ไม่อยู่ในช่วงมาตรฐาน ({std['ph_min']} - {std['ph_max']})")
                        if not ec_pass: st.info(f"📌 EC {ec_input:.2f} ต่ำกว่าเกณฑ์ (ควร >= {std['ec_min']})")
                        if not odor_pass: st.info(f"📌 กลิ่นระดับ {odor_score} ยังไม่ถึงระดับพร้อมใช้")

                    # Dashboard สรุปตัวเลข
                    m1, m2, m3 = st.columns(3)
                    m1.metric("ชนิดน้ำหมัก", detected_fruit)
                    m2.metric("ค่า pH ปัจจุบัน", f"{ph_input:.2f}")
        ...
        st.warning("กรุณาอัปโหลดรูปภาพและรอให้ระบบพร้อมใช้งาน")
# --- 6. ตารางอ้างอิง ---
